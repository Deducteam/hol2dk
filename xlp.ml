...
(* [export_one_file_by_prf b r] creates a lp file for each proof in
   range [r]. Warning: checking the generated lp files currently takes
   more times because of the way loading is currently done in Lambdapi
   (see https://github.com/Deducteam/lambdapi/issues/959). *)
let export_one_file_by_prf b r =
  use_abbrev := false;
...
let gen_makefile_one_file_by_prf b i x y =
  ...
  let fname = "Makefile" ^ string_of_int i in
  log "generate %s ...\n%!" fname;
...
(* [gen_makefiles_one_file_by_prf basename nb_proofs nb_parts] creates
   several makefiles to translate to Coq the lp files generated by
   [export_one_file_by_prf basename]. *)
let gen_makefiles_one_file_by_prf b nb_proofs nb_parts =
  Xlib.iter_parts nb_proofs nb_parts (gen_makefile_one_file_by_prf b);
  let fname = "Makefile" in
  log "generate %s ...\n%!" fname;
  let oc = open_out fname in
  out oc "# file generated by: hol2dk mk-exp %d %s\n\
          .SUFFIXES:\n\
          default:\n" nb_parts b;
  for k = 1 to nb_parts do out oc "\t$(MAKE) -f Makefile%d\n" k done;
  out oc ".PHONY: clean\n\
          clean:\n\
