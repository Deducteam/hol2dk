...
(* [gen_makefile_one_file_by_prf b r] creates the file "Makefile" to
   translate to Coq and check the lp files generated by
   [export_one_file_by_prf b r]. *)
let gen_makefile_one_file_by_prf b r =
  ...
  in
  begin match r with
  | All -> iter_proofs_at handle_proof
  | Upto x -> for k=0 to x do handle_proof k (proof_at k) done
  | Only _ | Inter _ -> invalid_arg "export_one_file_by_prf"
  end;
