...
hol2dk exp $file
  generate a lp file for each proof step

hol2dk mk-exp $n $file
  generate $n Makefiles for translating to Coq and check in Coq the files
  generated by hol2dk exp $file
...
|} f f b b (olist ostring) (trans_file_deps (dep_graph (files())) f);
  ...
  | "exp"::basename::args ->
     ...
     read_pos basename;
     init_proof_reading basename;
     Xlp.export_one_file_by_prf basename r

  | ["mk-exp";nb_parts;basename] ->
     let nb_parts = integer nb_parts in
     if nb_parts < 1 then wrong_arg();
     read_pos basename;
     init_proof_reading basename;
     Xlp.gen_makefiles_one_file_by_prf basename (nb_proofs()) nb_parts
